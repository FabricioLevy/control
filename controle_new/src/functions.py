...
class ControlFunctions:
    ...
    def plot_poles(self, sys, output_path):
        ...
        # plt.figure()
        # plt.scatter(zeros.real, zeros.imag, s=50, marker='o', color='b', label='Zeros')
        # plt.scatter(poles.real, poles.imag, s=50, marker='x', color='r', label='Polos')
    ...
    def plot_poles_mult(self, sys_open, sys_closed, output_path):
        ...
        # plt.xlim(-0.05, 0.05)  
        # plt.ylim(np.min(np.imag(poles_open + poles_closed)) - 20, np.max(np.imag(poles_open + poles_closed)) + 20)  # Ajuste dos limites do eixo y conforme necessário
        plt.title('Polos em malhas aberta e fechada')
        plt.xlabel('Parte Real')
        plt.ylabel('Parte Imaginária')
        plt.grid()
        plt.legend()
    ...
    def calcular_ci(self, m_i, w_i, zeta):
        ci = 2 * m_i * w_i * zeta
        return ci
